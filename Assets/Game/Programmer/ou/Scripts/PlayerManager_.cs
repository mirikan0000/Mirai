...
public class PlayerManager_ : MonoBehaviour
{
    ...
    void Update()
    {
        if (is_start)
        {
            ...
            //弾丸発射
            if (Input.GetKeyUp(KeyCode.Space) && !isShotting) 
            {
                //発射
                //二つ方法(重力、三角関数で模擬放物線)
                //重力
            ...
            if (is_aiming && !isShotting) 
            {
                ...
                {
                    gun_rotAngle = (gun_rotAngle - gunBarrel_rotSpeed) > -10.0f ? (gun_rotAngle - gunBarrel_rotSpeed) : -10.0f;
                }
